...
<!--                 </div> -->
				...
				<div class="card-body">
				 ...
				 -->
			<div class="container" style="width:1128px">
				<div style="margin-top: 80px;padding-left:12px"> 
					<span>活動類別:</span>
					<select class="onchange">							
						<option ${eventType == '全部' ? 'selected' : ''}>全部</option>
						<option value="書友讀書會" ${eventType == '書友讀書會' ? 'selected' : ''}>書友讀書會</option>
						<option value="新書分享會" ${eventType == '新書分享會' ? 'selected' : ''}>新書分享會</option>
						<option value="展覽及講座" ${eventType == '展覽及講座' ? 'selected' : ''}>展覽及講座</option>
						<option value="其他" ${eventType == '其他' ? 'selected' : ''}>其他</option>
					</select>
				</div>
				<div class="row row-cols-1 row-cols-md-3 g-4" style="margin: 0px 0px;">
...
<!-- 	1昀新增的script -->
	<script>
		$(document).on('change','.onchange',function(){
			var eventType = $('.onchange').val();
			console.log(eventType);
			$.ajax({
				type:'get',
				url:'/eventbytype/'+eventType,
				success: function(data){
					console.log('ajax success!');
					location.href="/eventfrontpage0";
				},
				error: function(e){
					console.log('findbytype is wrong')
				}
