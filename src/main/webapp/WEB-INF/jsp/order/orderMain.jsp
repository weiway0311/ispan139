								...
								<table cellspacing="0" class="table table-striped">
									...
									<c:forEach items="${orders}" var="order">
										<tr>
											...
											<td>${order.custId}</td>
											<td>${order.total}</td>
											<td id="status${order.oId}">${order.status}</td>
											<td><c:if test="${order.status!='已取消' && order.status!='已完成'}">
													<span id="updateButton${order.oId}">
														<button class="btn btn-outline-warning" onclick="submitUpdate(${order.oId},'${order.status}')">修改</button>
													</span>
												</c:if>
												<a class="btn btn-outline-success" href="gotodetails.controller?id=${order.oId}">詳情</a>
											</td>
										</tr>
									</c:forEach>
								</table>
		...
		function genderChart(s0, s1, s2, s3, s4){
			var genderChart = new Chart($("#genderChart"), {
			  ...
			  data: {
			    ...
			    datasets: [{
			      data: [s0, s1, s2, s3, s4],
			      backgroundColor: ['#FFBB66', '#66FF66', '#33CCFF', '#9999FF', '#FF8888'],
			      hoverBackgroundColor: ['#e6af41', '#55AA00', '#2c9faf', 'black', '#C63300'],
